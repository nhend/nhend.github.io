...
def pick_file():
    """ Pick input file by opening dialog window. Calls write_out() with JSON """
    filename = filedialog.askopenfile(mode='r', filetypes=[("JSON File", "*.json")])
...
def main():
    ...
    msg += "Would you like to convert an existing JSON file" \
          "\nor get current data from the database?"
    question = tk.Label(root, text=msg)
    question.grid(row=0, column=1, pady=(15, 10), padx=15, columnspan=3)

    button_file = tk.Button(root, text="Pick file",
                            command=lambda: [pick_file(), root.destroy()])
    button_file.grid(row=1, column=1, pady=10, padx=(15, 5))
